...
function ControlBar({ onAddTable, onAddSeat, selTableId, selSeatId, tables, onDeleteSeat, onDeleteTable, onStatusChange, onDeselect, onSave, saved }) {
  ...
  return (
    <div style={{ background:"#1B2A4A", borderRadius:"0 0 14px 14px", padding:"16px 20px 18px", fontFamily:F.body }}>
      ...
      {(hasSelT || hasSelS) && (
        <div style={{ marginTop:10, padding:"13px 16px", background:"rgba(255,255,255,0.04)", borderRadius:10, border:"1px solid rgba(255,255,255,0.07)" }}>
      ...
      <p style={{ margin:"12px 0 0", fontFamily:F.body, fontSize:10, color:"rgba(255,255,255,0.2)", lineHeight:1.6, letterSpacing:0.2 }}>
        {hasSelS
          ...
          : hasSelT
          ? "Drag table to move · Drag edge handles to resize · Press Save & Publish when done"
...
export default function SeatMap({
  ...
  onTablesChange,
  onUpdate,
  // For persistence: which room this map belongs to
  wing = "Main Wing",
  ...
  const [tables,     setTables]     = useState(() => rawTables.map((t,i) => hydrateTable(t,i)));
