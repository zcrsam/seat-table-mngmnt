// src/features/client/pages/TowerWing/GrandBallroomC.jsx
...
function ModalSuccess({ refCode, onBack, mode, guests, isRebook, bookingDetails, C, isDark }) {
  ...
  const handleSavePhoto = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.download = `bellevue-grand-ballroom-c-${refCode || "ticket"}.png`; link.href = canvas.toDataURL("image/png"); link.click();
...
export default function GrandBallroomC() {
  ...
  const fetchAndMerge = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        console.error("[GrandBallroomC] Reservations API failed:", res.status, apiUrl);
        return;
      }
      const data = await res.json();

      const reservations = Array.isArray(data) ? data : (data.data || []);
      console.log("[GrandBallroomC] reservations from API:", reservations);

      // Build a multi-key status map
      const seatStatusMap = {};
      ...
      console.log("[GrandBallroomC] seatStatusMap:", seatStatusMap);
      ...
      console.error("[GrandBallroomC] Failed to fetch reservations:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("[GrandBallroomC] WebSocket initialization failed:", err);
      startPolling();
      return () => stopPolling();
    }
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", background: isDark ? "rgba(10,9,8,0.85)" : "rgba(247,244,238,0.90)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.borderAccent}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'Playfair Display','Times New Roman',serif", fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Grand Ballroom C</div>
              </div>
            </div>
