// src/features/client/pages/TowerWing/Tower2.jsx
...
// ─── Normalize API status — approved/reserved → "reserved" (red) ─────────────
...
const apiCall = async (endpoint, options = {}) => {
  try {
    ...
    if (isCreateReservation) {
      console.error("[Tower2] POST /reservations failed — falling back to offline:", error.message);
...
// ─── CloseBtn — same as Tower1 (hardcoded dark stroke, works on dark modal bg) ─
...
// ─── ModalShell — same as Tower1 (dark surface, dark border) ─────────────────
function ModalShell({ children, onClose, disabled, C, maxWidth = 500 }) {
  return (
    ...
    >
      <div style={{ background: "#111009", borderRadius: 14, width: "100%", maxWidth, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.50)", border: "1px solid rgba(255,255,255,0.08)", fontFamily: F.body, position: "relative", animation: "modalIn 0.20s cubic-bezier(0.16,1,0.3,1)", overflow: "hidden" }}>
...
// ─── ModalHeader — same as Tower1 (hardcoded dark gradient, dark text) ────────
function ModalHeader({ eyebrow, title, onClose, disabled, C, meta }) {
  return (
    <div style={{ background: "linear-gradient(160deg,#111009 0%,#161410 100%)", padding: "20px 22px 18px", position: "sticky", top: 0, zIndex: 2, borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
...
// ─── Step Indicator — same as Tower1 (hardcoded dark colors) ─────────────────
    ...
    return (
      <ModalShell onClose={onCancel} C={C}>
        ...
        <div style={{ padding: "22px 24px 26px" }}>
          <div style={{ background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)", borderRadius: 10, overflow: "hidden", marginBottom: 22 }}>
            {[
              ["Room", ROOM, null],
              ["Seat Number", `Seat ${seatData?.num ?? seatData?.id ?? "—"}`, null],
              ["Availability", seatData?.status === "available" ? "Available" : "Unavailable",
              ...
              <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 16px", borderBottom: i < arr.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <span style={{ fontFamily: F.label, fontSize: 9, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(237,232,223,0.32)" }}>{key}</span>
                <span style={{ fontFamily: F.body, fontSize: 13, fontWeight: 600, color: color || "#EDE8DF" }}>{val}</span>
              </div>
            ))}
          </div>
        ...
        {mode === "individual" && (
          <div style={{ background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)", borderRadius: 10, overflow: "hidden", marginBottom: 22 }}>
            {infoRows.map(([key, val, color], i) => (
              <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 16px", borderBottom: i < infoRows.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <span style={{ fontFamily: F.label, fontSize: 9, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(237,232,223,0.32)" }}>{key}</span>
                <span style={{ fontFamily: F.body, fontSize: 13, fontWeight: 600, color: color || "#EDE8DF" }}>{val}</span>
              </div>
            ))}
        ...
        {mode === "whole" && (
          <>
            <div style={{ textAlign: "center", marginBottom: 22 }}>
              <div style={{ fontFamily: F.label, fontSize: 9, letterSpacing: "0.22em", color: "#8A8278", fontWeight: 700, textTransform: "uppercase", marginBottom: 14 }}>Number of Guests</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0, marginBottom: 10 }}>
                <button onClick={dec} disabled={atMin}
                  style={{ width: 44, height: 52, border: `1.5px solid ${atMin ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.08)"}`, borderRight: "none", borderRadius: "8px 0 0 8px", background: "rgba(255,255,255,0.04)", color: atMin ? "rgba(237,232,223,0.32)" : C.gold, fontSize: 20, fontWeight: 700, cursor: atMin ? "not-allowed" : "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", justifyContent: "center", opacity: atMin ? 0.4 : 1 }}
                ...
                <input type="text" inputMode="numeric" pattern="[0-9]*" value={inputVal} onChange={handleInputChange} onBlur={handleInputBlur}
                  style={{ width: 80, height: 52, border: `1.5px solid rgba(196,163,90,0.30)`, borderLeft: "none", borderRight: "none", background: "rgba(255,255,255,0.04)", textAlign: "center", fontFamily: F.display, fontSize: 28, fontWeight: 700, color: "#EDE8DF", outline: "none", colorScheme: "dark", MozAppearance: "textfield", WebkitAppearance: "none", boxSizing: "border-box" }}
                />
                <button onClick={inc} disabled={atMax}
                  style={{ width: 44, height: 52, border: `1.5px solid ${atMax ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.08)"}`, borderLeft: "none", borderRadius: "0 8px 8px 0", background: "rgba(255,255,255,0.04)", color: atMax ? "rgba(237,232,223,0.32)" : C.gold, fontSize: 20, fontWeight: 700, cursor: atMax ? "not-allowed" : "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", justifyContent: "center", opacity: atMax ? 0.4 : 1 }}
                  onMouseEnter={e => { if (!atMax) e.currentTarget.style.background = C.goldFaint; }}
                  onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}
                >+</button>
              </div>
              <div style={{ fontFamily: F.body, fontSize: 12, color: "#8A8278", lineHeight: 1.6 }}>
                Table <strong style={{ color: "#EDE8DF" }}>{tableData?.id || "T1"}</strong> has{" "}
                <strong style={{ color: "#EDE8DF" }}>{capacity} available seat{capacity !== 1 ? "s" : ""}</strong>
                {pendingSeats.length > 0 && <span style={{ color: C.gold }}>{" "}({pendingSeats.length} pending approval)</span>}
              </div>
              {atMax && (
                <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 7, background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)", fontFamily: F.body, fontSize: 11.5, color: C.gold, lineHeight: 1.5 }}>
                  Maximum reached — only <strong>{capacity}</strong> seat{capacity !== 1 ? "s" : ""} available on this table.
                </div>
              )}
            </div>
            <div style={{ padding: "12px 16px", borderRadius: 8, marginBottom: 20, background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)" }}>
              <div style={{ fontFamily: F.label, fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", color: "rgba(237,232,223,0.32)", textTransform: "uppercase", marginBottom: 4 }}>Seats to be Reserved</div>
...
function ModalDetails({ tableData, seatData, mode, guests, isStandalone, onReview, onCancel, prefill, C, isDark, secondsLeft, onTimerExpired }) {
  ...
  // Use dark-only field style inside modal (always dark modal)
  const darkC = {
    ...C,
    surfaceInput: "rgba(255,255,255,0.04)",
  ...
  return (
    <ModalShell onClose={onCancel} C={C}>
      ...
      <div style={{ padding: "18px 24px 26px", maxHeight: "64vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: 8, marginBottom: 16, background: isUrgent ? "rgba(184,92,92,0.05)" : "rgba(196,163,90,0.04)", border: `1px solid ${isUrgent ? "rgba(184,92,92,0.15)" : "rgba(196,163,90,0.30)"}` }}>
          <div>
            <div style={{ fontFamily: F.label, fontSize: 9, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: isUrgent ? C.red : "#8A8278", marginBottom: 2 }}>Seat Hold Timer</div>
            <div style={{ fontFamily: F.body, fontSize: 11, color: isUrgent ? C.red : "rgba(237,232,223,0.32)" }}>{isUrgent ? "⚠ Hold expiring soon" : "Complete before the timer expires"}</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 0, marginBottom: 20, borderRadius: 8, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
          {summaryColumns.map(([label, value], i, arr) => (
            <div key={label} style={{ flex: 1, padding: "10px 12px", background: "rgba(255,255,255,0.04)", borderRight: i < arr.length - 1 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
              <div style={{ fontFamily: F.label, fontSize: 8, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(237,232,223,0.32)", marginBottom: 3 }}>{label}</div>
              <div style={{ fontFamily: F.body, fontSize: 11, fontWeight: 600, color: label === "Seat" ? C.gold : "#EDE8DF", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</div>
            </div>
          ))}
        </div>
        <SectionLabel C={darkC}>Personal Information</SectionLabel>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="First Name" value={form.firstName} onChange={set("firstName")} C={darkC} isDark={true} required />
          <Field label="Last Name"  value={form.lastName}  onChange={set("lastName")}  C={darkC} isDark={true} required />
        </div>
        <Field label="Email Address" value={form.email} onChange={set("email")} type="email" C={darkC} isDark={true} required />
        <Field label="Phone Number"  value={form.phone} onChange={set("phone")} type="tel" C={darkC} isDark={true} required placeholder="+63XXXXXXXXXX" />
        <SectionLabel C={darkC} style={{ marginTop: 4 }}>Event Details</SectionLabel>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
...
function ModalReview({ form, guests, tableData, seatData, mode, isStandalone, onSubmit, onEdit, submitting, isRebook, rebookFrom, C }) {
  ...
  const Row = ({ label, value, accent }) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
      <span style={{ fontFamily: F.label, fontSize: 9, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(237,232,223,0.32)", minWidth: 90, flexShrink: 0 }}>{label}</span>
      <span style={{ fontFamily: F.body, fontSize: 12.5, color: accent ? C.gold : "#EDE8DF", fontWeight: accent ? 700 : 500, textAlign: "right", maxWidth: 260, lineHeight: 1.5 }}>{value}</span>
    </div>
  );

  const darkC = { ...C, divider: "rgba(255,255,255,0.05)" };

  return (
    <ModalShell onClose={onEdit} disabled={submitting} C={C}>
      <ModalHeader
        eyebrow={isRebook ? "Rebook / Move Seat" : isStandalone ? "Seat Reservation" : mode === "individual" ? "Seat Reservation" : "Table Reservation"}
      ...
      <div style={{ padding: "20px 24px 26px", maxHeight: "64vh", overflowY: "auto" }}>
        {isRebook && rebookFrom && (
          <div style={{ padding: "11px 14px", borderRadius: 8, marginBottom: 18, background: "rgba(196,163,90,0.05)", border: "1px solid rgba(196,163,90,0.15)", fontSize: 12, color: C.gold, lineHeight: 1.65 }}>
        ...
        <SectionLabel C={darkC}>Reservation Details</SectionLabel>
        {reservationRows.map(([k, v]) => <Row key={k} label={k} value={v} accent={k === "Seat(s)"} />)}
        <SectionLabel C={darkC} style={{ marginTop: 18 }}>Guest Information</SectionLabel>
        {guestRows.map(([k, v]) => <Row key={k} label={k} value={v} />)}
        <div style={{ padding: "10px 14px", borderRadius: 8, margin: "18px 0 20px", background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)", fontSize: 11.5, color: "#8A8278", lineHeight: 1.65 }}>
          Your booking will be <strong style={{ color: "#EDE8DF" }}>pending admin review</strong> upon submission.
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onEdit} disabled={submitting}
            style={{ flex: 1, padding: "12px", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, background: "transparent", color: "#8A8278", fontFamily: F.label, fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", cursor: submitting ? "not-allowed" : "pointer", transition: "all 0.18s" }}
            onMouseEnter={e => { if (!submitting) { e.currentTarget.style.borderColor = "rgba(196,163,90,0.30)"; e.currentTarget.style.color = C.gold; } }}
            onMouseLeave={e => { if (!submitting) { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.color = "#8A8278"; } }}
          >Edit Details</button>
          <button onClick={onSubmit} disabled={submitting}
            style={{ flex: 2, padding: "12px", border: "none", borderRadius: 8, background: submitting ? "#8A8278" : C.gold, color: C.textOnAccent, fontFamily: F.label, fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", cursor: submitting ? "not-allowed" : "pointer", transition: "all 0.18s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
...
const buildQrValue = ({ refCode }) => {
  ...
  return `${url}/tower2-reserve/${String(refCode || "").trim()}`;
};
...
function ModalSuccess({ refCode, onBack, mode, guests, isRebook, bookingDetails, C, isDark }) {
  ...
  const handleSavePhoto = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.download = `bellevue-tower2-${refCode || "ticket"}.png`; link.href = canvas.toDataURL("image/png"); link.click();
    ...
    <ModalShell onClose={onBack} C={C} maxWidth={460}>
      <div style={{ padding: "26px 26px 28px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={{ width: 40, height: 40, borderRadius: 8, flexShrink: 0, background: "rgba(74,158,126,0.05)", border: "1px solid rgba(74,158,126,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div>
            ...
            <div style={{ fontFamily: F.display, fontSize: 22, fontWeight: 600, color: "#EDE8DF", lineHeight: 1.2 }}>Pending Approval</div>
          </div>
        </div>
        <div style={{ padding: "14px 16px", borderRadius: 10, marginBottom: 16, background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)" }}>
          <div style={{ fontFamily: F.label, fontSize: 8, letterSpacing: "0.20em", fontWeight: 700, textTransform: "uppercase", color: "rgba(237,232,223,0.32)", marginBottom: 6 }}>Reference Code</div>
          <div style={{ fontFamily: F.mono, fontSize: 24, fontWeight: 800, color: "#EDE8DF", letterSpacing: "0.12em" }}>{refCode || "—"}</div>
        </div>
        <div style={{ display: "flex", gap: 14, marginBottom: 20 }}>
          <div style={{ flex: 1 }}>
            {[{ label: "Table", value: bookingDetails?.table || "—" }, { label: "Date", value: fmtDate(bookingDetails?.date) }, { label: "Guests", value: String(guests) }, { label: "Status", value: "Pending Review", gold: true }].map(({ label, value, gold }, i, arr) => (
              <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < arr.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <span style={{ fontFamily: F.label, fontSize: 8, fontWeight: 700, letterSpacing: "0.14em", color: "rgba(237,232,223,0.32)", textTransform: "uppercase" }}>{label}</span>
                <span style={{ fontFamily: F.body, fontSize: 12, fontWeight: 600, color: gold ? C.gold : "#EDE8DF" }}>{value}</span>
              </div>
            ))}
          </div>
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <div style={{ padding: 8, background: "#FFFFFF", borderRadius: 8, border: "1px solid rgba(255,255,255,0.08)" }}>
              <QRCodeWithRef value={qrValue} size={88} imgRef={qrImgRef} />
            </div>
            <div style={{ fontFamily: F.label, fontSize: 8, color: "rgba(237,232,223,0.32)", letterSpacing: "0.06em" }}>Scan to verify</div>
          </div>
        </div>
        <div style={{ fontFamily: F.body, fontSize: 12, color: "#8A8278", lineHeight: 1.65, padding: "10px 14px", borderRadius: 8, background: "rgba(196,163,90,0.04)", border: "1px solid rgba(196,163,90,0.30)", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={handleSavePhoto} disabled={saving || !qrReady}
            style={{ flex: 1, padding: "12px", background: "transparent", border: `1px solid ${(saving || !qrReady) ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.12)"}`, borderRadius: 8, fontFamily: F.label, fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: (saving || !qrReady) ? "rgba(237,232,223,0.32)" : "#8A8278", cursor: (saving || !qrReady) ? "not-allowed" : "pointer", transition: "all 0.18s", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}
            onMouseEnter={e => { if (!saving && qrReady) { e.currentTarget.style.borderColor = "rgba(196,163,90,0.30)"; e.currentTarget.style.color = C.gold; } }}
            onMouseLeave={e => { if (!saving && qrReady) { e.currentTarget.style.borderColor = "rgba(255,255,255,0.12)"; e.currentTarget.style.color = "#8A8278"; } }}
          >
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 200, background: C.bottomSheet, borderTop: `1px solid ${C.borderAccent}`, borderRadius: "20px 20px 0 0", boxShadow: "0 -8px 32px rgba(0,0,0,0.28)", padding: "0 0 max(env(safe-area-inset-bottom), 12px) 0", animation: "slideUp 0.26s cubic-bezier(0.16,1,0.3,1)" }}>
      ...
      <div style={{ padding: "10px 16px 14px" }}>
        ...
        <button onClick={canGo ? onReserve : undefined} disabled={!canGo}
          style={{ width: "100%", padding: "15px", background: canGo ? C.gold : (isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"), border: "none", borderRadius: 12, fontFamily: F.label, fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: canGo ? C.textOnAccent : C.textTertiary, cursor: canGo ? "pointer" : "not-allowed", transition: "all 0.20s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
...
export default function Tower2() {
  ...
  // ─── KEY FIX: Same fetchAndMerge approach as Tower1 ──────────────────────────
  // Read from /reservations endpoint so approved → "reserved" (red) always works
  const fetchAndMerge = useCallback(async () => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/reservations?room=${encodeURIComponent(ROOM)}&wing=${encodeURIComponent(WING)}&venue_id=3&per_page=9999`,
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (!fbData?.data) return;
          setTableData(prev => {
            const base = prev || loadLayoutForClient(WING, ROOM);
            const merged = base ? mergeApiStatusIntoLayout(base, fbData.data) : fbData.data;
      ...
      // Filter to this room/wing only
      const reservations = rows.filter(r =>
        !r.room || r.room === ROOM || r.wing === WING
      );
      ...
      reservations.forEach(r => {
        const apiStatus = normaliseApiStatus(r.status);
        const isStandalone =
          String(r.table_number ?? "").toUpperCase() === "STANDALONE" ||
          r.type === "standalone" ||
          r.is_standalone === 1 ||
          r.is_standalone === true;

        const tableKey = isStandalone
          ? "STANDALONE"
          : String(r.table_number ?? "").trim();

        const rawSeats = String(r.seat_number ?? r.seat ?? "").trim();
        if (!rawSeats) return;

        rawSeats.split(",").map(s => s.trim()).filter(Boolean).forEach(seatNum => {
          const key = `${tableKey}|${seatNum}`;
          const existing = seatStatusMap[key];
          if (!existing || (priority[apiStatus] ?? -1) > (priority[existing] ?? -1)) {
            seatStatusMap[key] = apiStatus;
          }
        });
      });
      ...
      setTableData(prev => {
        ...
        const updatedTables = (base.tables || []).map(t => {
          const tId = String(t.id ?? "").trim();
          const tIdNorm = tId.replace(/^T/i, "");
          return {
            ...t,
            seats: (t.seats || []).map(s => {
        ...
        const updatedStandalone = (base.standaloneSeats || []).map(s => {
          const sNum = String(s.num ?? s.id ?? "").trim();
          const resolved =
      ...
      console.error("[Tower2] fetchAndMerge error:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const events = [
        ...
        ".ReservationApproved", ".ReservationUpdated", ".ReservationStatusUpdated",
      ];
    ...
    <ThemeContext.Provider value={{ isDark, toggle: toggleTheme }}>
      ...
      <div style={{ minHeight: "100vh", fontFamily: FONT, background: C.pageBg, transition: "background 0.30s", position: "relative" }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 0 }}>
          <div style={{ position: "absolute", inset: 0, backgroundImage: `url('${twentyTwentyImg}')`, backgroundSize: "cover", backgroundPosition: "center", filter: isDark ? "blur(6px) brightness(0.35)" : "blur(6px) brightness(0.45) saturate(0.4)", transform: "scale(1.05)", transition: "filter 0.40s" }} />
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", background: isDark ? "rgba(10,9,8,0.85)" : "rgba(247,244,238,0.90)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.borderAccent}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'Playfair Display','Times New Roman',serif", fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Tower 2 Ballroom</div>
              </div>
            </div>
            ...
            <div style={{ width: "100%", height: mobileMapHeight, position: "relative", background: C.surfaceBase, overflow: "hidden" }}>
              {tableData ? (
                <>
                  <div style={{ width: "100%", height: "100%", overflow: "auto", WebkitOverflowScrolling: "touch", position: "relative" }}>
                    <div style={{ minWidth: 700, minHeight: Math.max(mobileMapHeight, 520), position: "relative" }}>
                      <SeatMap tableData={tableData} editMode={false} selectedSeat={selectedSeat} onSeatClick={handleSeatClick} onTableClick={handleTableClick} windowWidth={700} wing={WING} room={ROOM} mode={mode} />
                    </div>
                  </div>
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto", padding: isTablet ? "28px 24px" : "36px 48px" }}>
              ...
              <div style={{ marginBottom: 28, animation: "fadeUp 0.32s ease" }}>
                ...
                <p style={{ fontFamily: FONT, fontSize: 13.5, color: C.textSecondary, margin: 0, lineHeight: 1.70, maxWidth: 560 }}>
                  Book your preferred table in Tower 2 Ballroom. Select your reservation type and click on the map to get started.
                </p>
              </div>
              ...
              <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexDirection: isTablet ? "column" : "row", animation: "fadeUp 0.36s ease" }}>
                  ...
                  {tableData ? (
                    <>
                      <div style={{ flex: 1, minHeight: 0, position: "relative" }}>
                        <SeatMap tableData={tableData} editMode={false} selectedSeat={selectedSeat} onSeatClick={handleSeatClick} onTableClick={handleTableClick} windowWidth={windowSize.width} wing={WING} room={ROOM} mode={mode} />
                      </div>
                ...
                <div style={{ width: isTablet ? "100%" : 280, flexShrink: 0, display: "flex", flexDirection: "column", gap: 14 }}>
                  <div style={{ display: isTablet ? "grid" : "flex", gridTemplateColumns: isTablet ? "1fr 1fr" : undefined, flexDirection: isTablet ? undefined : "column", gap: 14 }}>
                    ...
                    {/* Selection summary — same as Tower1 */}
                    <div style={{ background: C.surfaceBase, borderRadius: 12, border: `1px solid ${C.borderDefault}`, overflow: "hidden", boxShadow: isDark ? "0 4px 20px rgba(0,0,0,0.30)" : "0 2px 12px rgba(0,0,0,0.06)" }}>
                      <div style={{ height: "2px", background: `linear-gradient(90deg, transparent 0%, ${C.gold}60 50%, transparent 100%)` }} />
                      <div style={{ padding: "14px 16px" }}>
