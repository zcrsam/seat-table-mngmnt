...
// ── FIXED: always resets every seat to "available" before applying API statuses
// so stale localStorage values (e.g. "pending") can never survive an API refresh
function mergeApiStatusIntoLayout(localLayout, apiData) {
  ...
  const mergedTables = (localLayout.tables || []).map(t => ({
    ...t,
    seats: (t.seats || []).map(s => {
      // Always default to "available" — API is sole source of truth for status
      const apiStatus =
        apiStatusMap[s.id] ??
...
const buildQrValue = ({ refCode }) => {
  ...
  return `${url}/tower3-reserve/${String(refCode || "").trim()}`;
};
...
function ModalSuccess({ refCode, onBack, mode, guests, isRebook, bookingDetails, C }) {
  ...
  const handleSavePhoto = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.download = `bellevue-tower3-reservation-${refCode || "ticket"}.png`; link.href = canvas.toDataURL("image/png"); link.click();
    ...
    <ModalShell onClose={onBack} C={C} maxWidth={460}>
      <div style={{ padding: "26px 26px 28px" }}>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 20 }}>
          <div style={{ flex: 1 }}>
            {[{ label: "Room", value: ROOM }, { label: "Date", value: fmtDate(bookingDetails?.date) }, { label: "Guests", value: String(guests) }, { label: "Status", value: "Pending Review", gold: true }].map(({ label, value, gold }, i, arr) => (
      ...
      <div style={{ padding: "10px 16px 14px" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1.4, padding: "8px 12px", borderRadius: 10, background: C.surfaceInput, border: `1px solid ${C.borderDefault}` }}>
            ...
            <div style={{ fontFamily: F.body, fontSize: 11, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Tower 3</div>
          </div>
        </div>
        <button
          ...
          style={{
            ...
            cursor: canGo ? "pointer" : "not-allowed", transition: "all 0.18s",
            display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
          }}
        >
          {mode === "whole"
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function Tower3Reserve() {
  ...
  // ─── KEY FIX: Same fetchAndMerge approach as Tower1 & Tower2 ─────────────────
  ...
  const fetchAndMerge = useCallback(async () => {
    try {
      ...
      console.error("[Tower3] fetchAndMerge error:", err);
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Optimistic update — mark as pending immediately in UI
      setTableData(prev => {
        if (!prev) return prev;
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Tower 3</div>
              </div>
            </div>
              ...
              {tableData ? (
                ...
                <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, padding: 32 }}>
                  ...
                  <div style={{ fontFamily: F.body, fontSize: 13, color: C.textSecondary, textAlign: "center", lineHeight: 1.7 }}>
                    No seat layout published for Tower 3.<br />
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto", padding: isTablet ? "28px 24px" : "36px 48px" }}>
              ...
              <div style={{ marginBottom: 28, animation: "fadeUp 0.32s ease" }}>
                ...
                <p style={{ fontFamily: F.body, fontSize: 13.5, color: C.textSecondary, margin: 0, lineHeight: 1.70, maxWidth: 560 }}>
                  Book your preferred table in Tower Wing. Select your reservation type and click on the map to get started.
                </p>
              </div>
                ...
                <div style={{ flex: "1 1 0", width: isTablet ? "100%" : undefined, minWidth: 0, minHeight: 520, background: C.surfaceBase, borderRadius: 14, border: `1px solid ${C.borderDefault}`, overflow: "hidden", boxShadow: isDark ? "0 8px 40px rgba(0,0,0,0.40)" : "0 4px 24px rgba(0,0,0,0.08)", position: "relative", display: "flex", flexDirection: "column" }}>
                  ...
                  {tableData ? (
                    ...
                    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, padding: 48 }}>
                      ...
                      <div style={{ fontFamily: F.body, fontSize: 13, color: C.textSecondary, textAlign: "center", lineHeight: 1.7 }}>
                        No seat layout has been published for Tower 3 yet.<br />
                    ...
                    <div style={{ background: C.surfaceBase, borderRadius: 12, border: `1px solid ${C.borderDefault}`, overflow: "hidden", boxShadow: isDark ? "0 4px 20px rgba(0,0,0,0.30)" : "0 2px 12px rgba(0,0,0,0.06)" }}>
                      ...
                      <div style={{ padding: "14px 16px" }}>
                        ...
                        {[
                          ...
                          ...(!isStandalone ? [["Wing", WING, false, null]] : []),
