// src/features/client/pages/TowerWing/GrandBallroomB.jsx
...
const WING = "Grand Ballroom";
const ROOM = "Grand Ballroom B";
...
// "approved" → "reserved" (red), "pending" → "pending" (orange/gold),
// "rejected" / "cancelled" → "available", anything else passes through.
...
// ─── Helper: is a seat standalone (not inside any table)? ───────────────
function isStandaloneSeat(seat, tables = []) {
  if (!seat) return false;
  return !tables.some(t => t.seats?.some(s => s.id === seat.id));
}
...
function ModalSuccess({ refCode, onBack, mode, guests, isRebook, bookingDetails, C, isDark }) {
  ...
  const handleSavePhoto = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.download = `bellevue-grand-ballroom-b-${refCode || "ticket"}.png`; link.href = canvas.toDataURL("image/png"); link.click();
...
export default function GrandBallroomB() {
  ...
  // Listen for admin layout saves
  useEffect(() => {
    const onStorage = e => {
      if (e.key !== layoutKey(WING, ROOM)) return;
      try { const parsed = e.newValue ? JSON.parse(e.newValue) : null; if (parsed) setTableData(parsed); } catch {}
    };
    const onSeatMapSaved = e => {
      if (e.detail?.wing !== WING || e.detail?.room !== ROOM) return;
      try { const parsed = e.detail.payload ? JSON.parse(e.detail.payload) : null; if (parsed) setTableData(parsed); } catch {}
    };
  ...
  // ─── THE KEY FIX: fetchAndMerge uses /reservations endpoint (same as Tower 1)
  // instead of /rooms/3/seats — this is what makes status colours work correctly.
  const fetchAndMerge = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        console.error("[GrandBallroomB] Reservations API failed:", res.status, apiUrl);
        return;
      }
      ...
      console.log("[GrandBallroomB] reservations from API:", reservations);

      // ── Build a multi-key status map so we can match seats regardless of
      //    how the backend stores table_number / seat_number ──────────────────
      const seatStatusMap = {};

      reservations.forEach(r => {
        const rawStatus = normaliseApiStatus(r.status);
        const tableKey  = String(r.table_number ?? "").trim();
        const seatNums  = String(r.seat_number ?? "")
        ...
        seatNums.forEach(seatNum => {
          // composite key
          if (tableKey && !isStandaloneRow) {
            seatStatusMap[`${tableKey}|${seatNum}`] = rawStatus;
          }
          // bare seat number key (fallback)
          seatStatusMap[seatNum] = rawStatus;
          // standalone-prefixed key
          if (isStandaloneRow) {
            seatStatusMap[`STANDALONE|${seatNum}`] = rawStatus;
          }
        });

        // Also index by DB seat_id if present
        if (seatId) seatStatusMap[`ID|${seatId}`] = rawStatus;
      });

      console.log("[GrandBallroomB] seatStatusMap:", seatStatusMap);

      setTableData(prev => {
        const base = prev || loadLayoutForClient(WING, ROOM);
        if (!base) return prev;

        // Helper: resolve status for any seat object inside a table
        const resolveTableSeatStatus = (t, s) => {
          const tableId  = String(t.id ?? t.label ?? t.tableId ?? "").trim();
          const seatNum  = String(s.num ?? s.label ?? s.id ?? "").trim();
          const seatDbId = s.db_id ? String(s.db_id).trim() : null;

          return (
            seatStatusMap[`${tableId}|${seatNum}`] ??
            seatStatusMap[seatNum] ??
            (seatDbId ? seatStatusMap[`ID|${seatDbId}`] : undefined) ??
            s.status
          );
        };

        // Helper: resolve status for standalone / root-level seats
        const resolveStandaloneSeatStatus = (s) => {
          const seatNum  = String(s.num ?? s.label ?? s.id ?? "").trim();
          const seatDbId = s.db_id ? String(s.db_id).trim() : null;
        ...
        const mergedTables = (base.tables || []).map(t => ({
          ...
          seats: (t.seats || []).map(s => ({
            ...s,
            status: resolveTableSeatStatus(t, s),
          })),
        }));

        const mergedStandaloneSeats = (base.standaloneSeats || []).map(s => ({
          ...s,
          status: resolveStandaloneSeatStatus(s),
        }));

        const mergedRootSeats = (base.seats || []).map(s => ({
          ...s,
          status: resolveStandaloneSeatStatus(s),
        }));
        ...
        const merged = {
          ...base,
          tables: mergedTables,
          standaloneSeats: mergedStandaloneSeats,
          ...(base.seats ? { seats: mergedRootSeats } : {}),
        };

        console.log("[GrandBallroomB] merged layout sample (first table seats):",
          merged.tables?.[0]?.seats?.slice(0, 4));
      ...
      console.error("[GrandBallroomB] Failed to fetch reservations:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("[GrandBallroomB] WebSocket initialization failed:", err);
      startPolling();
      return () => stopPolling();
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        type: seatIsStandalone ? "standalone" : mode,
        is_standalone: seatIsStandalone ? 1 : 0,
        seat_id: seatIsStandalone ? (selectedSeat?.id ?? null) : null,
      };
      ...
      setLastBookingDetails({
        room: ROOM,
        table: seatIsStandalone ? "Standalone Seat" : `Table ${activeTable?.id ?? "T1"}`,
      ...
      // Optimistic update — new reservation always starts as "pending"
      setTableData(prev => {
        if (!prev) return prev;

        if (seatIsStandalone && selectedSeat) {
          const updatedStandalone = (prev.standaloneSeats || []).map(s =>
            s.id === selectedSeat.id ? { ...s, status: "pending" } : s
          );
          const updatedRootSeats = (prev.seats || []).map(s =>
            s.id === selectedSeat.id ? { ...s, status: "pending" } : s
          );
          const updated = {
            ...prev,
            standaloneSeats: updatedStandalone,
            ...(prev.seats ? { seats: updatedRootSeats } : {}),
          };
      ...
      // Re-fetch immediately so the map reflects true server state
      await fetchAndMerge();
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", background: isDark ? "rgba(10,9,8,0.85)" : "rgba(247,244,238,0.90)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.borderAccent}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'Playfair Display','Times New Roman',serif", fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Grand Ballroom B</div>
              </div>
            </div>
            ...
            <MobileBottomSheet mode={mode} selectedSeat={selectedSeat} activeTable={activeTable} guests={guests} seatRatio={seatRatio} canProceed={canProceed} rebookFrom={rebookFrom} onReserve={() => setModal("guestCount")} C={C} isDark={isDark} isStandalone={seatIsStandalone} />
          </div>
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto", padding: isTablet ? "28px 24px" : "36px 48px" }}>
              ...
              <div style={{ marginBottom: 28, animation: "fadeUp 0.32s ease" }}>
                ...
                <p style={{ fontFamily: FONT, fontSize: 13.5, color: C.textSecondary, margin: 0, lineHeight: 1.70, maxWidth: 560 }}>
                  Book your preferred table in Grand Ballroom B. Select your reservation type and click on the map to get started.
                </p>
              </div>
