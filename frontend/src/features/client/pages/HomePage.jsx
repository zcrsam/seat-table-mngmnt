...
function getTokens(isDark) {
  return isDark
    ? {
        ...
        toggleTrack: "rgba(201,168,76,0.15)",
      }
    : {
        gold:        "#A07828",
        goldLight:   "#C9A84C",
        ...
        toggleTrack: "rgba(160,120,40,0.12)",
      };
}
...
// ─────────────────────────────────────────────
// THEME TOGGLE — inline pill button
// ─────────────────────────────────────────────
  ...
  return (
    <button
      ...
      style={{
        ...
        background: hov
          ? (isDark ? "rgba(201,168,76,0.12)" : "rgba(160,120,40,0.10)")
          : C.toggleTrack,
        cursor: "pointer",
    ...
    >
      ...
      <span style={{
        ...
        height: 22,
        borderRadius: "50%",
        background: isDark ? "rgba(201,168,76,0.20)" : "rgba(255,180,30,0.25)",
...
// ─────────────────────────────────────────────
// TOPBAR NAV — sits above the hero
// ─────────────────────────────────────────────
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${hov ? C.gold : C.border}`,
            background: hov
              ? (isDark ? "rgba(201,168,76,0.10)" : "rgba(160,120,40,0.08)")
              : "transparent",
            fontFamily: F.body,
            fontSize: 12,
            fontWeight: 600,
  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        textAlign: "center",
        padding: "clamp(120px,14vw,170px) clamp(24px,5vw,60px) clamp(40px,5vw,60px)",
      }}>
        <h1 style={{
          ...fade(0.10),
        ...
        <p style={{
          ...
          maxWidth: 460, margin: "0 auto 34px", fontWeight: 400, marginBottom: -10,
        }}>
      ...
      <div id="browse-wings" style={{
          ...
          {EVENT_CATEGORIES.map((cat) => (
            ...
            >
              <div style={{ height: "clamp(155px,15vw,205px)", overflow: "hidden", position: "relative" }}>
                <img src={cat.img} alt={cat.label} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.55s ease", transform: hovCard === cat.id ? "scale(1.07)" : "scale(1.01)" }} />
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.62) 0%, transparent 52%)", opacity: hovCard === cat.id ? 1 : 0.52, transition: "opacity 0.28s" }} />
              ...
              <div style={{ padding: "15px 18px 17px", borderTop: "1px solid rgba(201,168,76,0.09)", display: "flex", alignItems: "center", justifyContent: "space-between", background: hovCard === cat.id ? "rgba(201,168,76,0.05)" : "transparent", transition: "background 0.22s" }}>
                <div>
                ...
                <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, border: `1px solid ${hovCard === cat.id ? "#C9A84C" : "rgba(201,168,76,0.24)"}`, display: "flex", alignItems: "center", justifyContent: "center", background: hovCard === cat.id ? "#C9A84C" : "transparent", transition: "all 0.22s" }}>
                  <span style={{ fontSize: 13, color: hovCard === cat.id ? "#0E0D09" : "#C9A84C", display: "inline-block", transform: hovCard === cat.id ? "translateX(1px)" : "translateX(0)", transition: "transform 0.2s" }}>→</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ ...fade(0.50), maxWidth: 1160, margin: "18px auto 0", display: "flex", justifyContent: "flex-end" }}>
          <button type="button" onClick={() => onNavigateToVenues()} style={{ padding: "8px 20px", border: "1px solid rgba(201,168,76,0.26)", background: "transparent", color: "rgba(245,239,224,0.46)", fontFamily: F.body, fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", cursor: "pointer", borderRadius: 3, transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "#C9A84C"; e.currentTarget.style.color = "#C9A84C"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(201,168,76,0.26)"; e.currentTarget.style.color = "rgba(245,239,224,0.46)"; }}>
            View All Venues →
          </button>
        </div>
      </div>
      ...
      {/* Scroll cue */}
      <div style={{ position: "absolute", bottom: 24, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, opacity: loaded ? C.scrollCueOp : 0, transition: "opacity 1.2s 1.2s", pointerEvents: "none" }}>
...
function DiningSection({ onNavigate, initialRestaurantId }) {
  ...
  return (
    <section ref={ref} style={{ background: C.diningBg, padding: "clamp(64px,9vw,110px) clamp(20px,5vw,60px)", overflow: "hidden", transition: "background 0.35s ease" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(40px,6vw,80px)", alignItems: "start" }}>
          ...
          <div style={{ position: "relative", opacity: vis ? 1 : 0, transform: vis ? "none" : "translateX(-30px)", transition: "opacity 0.8s, transform 0.8s" }}>
            ...
            {/* Floating widget */}
            <div style={{ position: "absolute", bottom: -28, left: 24, right: 24, background: C.widgetBg, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${C.border}`, borderRadius: 10, padding: "22px 24px", boxShadow: isDark ? "0 20px 60px rgba(0,0,0,0.5)" : "0 20px 60px rgba(0,0,0,0.12)", transition: "background 0.35s, border-color 0.35s" }}>
          ...
          <div style={{ paddingTop: 10, opacity: vis ? 1 : 0, transform: vis ? "none" : "translateX(30px)", transition: "opacity 0.8s 0.15s, transform 0.8s 0.15s", position: "relative", zIndex: 10005 }}>
              ...
              {(restaurant?.diningTimes ?? DINING_TIMES).map((d) => {
                ...
                return (
                  <button key={d.label} type="button" onClick={() => selectDiningTime(d)}
                    style={{ padding: "7px 14px", borderRadius: 3, background: active ? C.gold : (isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"), border: `1px solid ${active ? C.gold : C.border}`, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "flex-start", position: "relative", zIndex: 10006, transition: "background 0.2s, border-color 0.2s" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {imgs.map((src, i) => (
                <div key={src} onClick={() => { setActiveImg(i); const pref = (restaurant?.diningTimes ?? DINING_TIMES)[0]; if (pref) selectDiningTime(pref); }}
...
function FieldInput({ type, value, onChange }) {
  ...
  return (
    <input type={type} value={value} onChange={(e) => onChange(e.target.value)}
      style={{ width: "100%", padding: "10px 13px", background: C.fieldBg, border: `1px solid ${C.border}`, borderRadius: 4, color: C.textPrimary, fontFamily: F.body, fontSize: 13, outline: "none", boxSizing: "border-box", colorScheme: isDark ? "dark" : "light", transition: "background 0.3s, border-color 0.3s, color 0.3s" }} />
  );
}
...
function GuestPicker({ value, onChange, min = 1, max = 20, style }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 13px", background: C.fieldBg, border: `1px solid ${C.border}`, borderRadius: 4, transition: "background 0.3s, border-color 0.3s", ...style }}>
      ...
      <div style={{ position: "relative", maxWidth: 1100, margin: "0 auto", padding: "clamp(50px,7vw,80px) clamp(20px,5vw,60px)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40, flexWrap: "wrap", opacity: vis ? 1 : 0, transition: "opacity 0.8s" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "flex-end" }}>
          <div style={{ display: "flex", overflow: "hidden", borderRadius: 3, border: `1px solid ${C.border}` }}>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" style={{ padding: "13px 20px", background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)", border: "none", color: C.textPrimary, fontFamily: F.body, fontSize: 14, outline: "none", width: 260, transition: "background 0.3s, color 0.3s" }} />
            <button type="button" onClick={() => { if (!email) return; setSent(true); setTimeout(() => { setSent(false); setEmail(""); }, 2500); }} style={{ padding: "13px 18px", background: sent ? C.gold : (isDark ? "#0E0D09" : "#1A1612"), border: "none", cursor: "pointer", color: sent ? (isDark ? "#0E0D09" : "#FFFFFF") : C.gold, transition: "all 0.25s", fontFamily: F.body, fontSize: 16 }}>
              {sent ? "✓" : "→"}
            </button>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {[["f","Facebook","https://www.facebook.com/thebellevuemanila/"],["t","X","https://x.com/bellevuemanila"],["i","Instagram","https://www.instagram.com/bellevuemanila/"],["y","YouTube","https://www.youtube.com/channel/UC01W6kRH_R-T0ok6RDT3aGg"]].map(([icon,label,href]) => (
              <a key={label} href={href} target="_blank" rel="noreferrer" title={label} style={{ width: 36, height: 36, borderRadius: "50%", border: `1px solid ${C.border}`, color: C.textMuted, fontFamily: F.body, fontSize: 14, fontWeight: 700, transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr", gap: 40, marginBottom: 56 }}>
          <div>
            <div style={{ marginBottom: 18 }}>
              <img src={bellevueLogo} alt="The Bellevue Manila" style={{ height: 42, width: "auto", display: "block", filter: isDark ? "none" : "brightness(0) saturate(100%) invert(30%) sepia(50%) saturate(400%) hue-rotate(5deg)", transition: "filter 0.35s" }} />
            </div>
            <p style={{ fontFamily: F.body, fontSize: 14, color: C.textFaint, lineHeight: 1.85, margin: "0 0 16px", transition: "color 0.35s" }}>Luxury event spaces and seamless reservations in the heart of Alabang.</p>
            <button type="button" onClick={onManageBooking}
              style={{ padding: "9px 18px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: 3, fontFamily: F.body, fontSize: 12, fontWeight: 600, color: C.gold, cursor: "pointer", letterSpacing: "0.08em", transition: "all 0.2s" }}
...
export default function HomePage() {
  ...
  // Navigate to ManageBooking page — route: /manage-booking
  const goToManageBooking = () => navigate("/manage-booking");

  useEffect(() => {
    const headerH = 68; const markerY = headerH + 8;
  ...
  return (
    <ThemeContext.Provider value={{ isDark, toggle: toggleTheme }}>
      <div style={{ background: C.pageBg, minHeight: "100vh", transition: "background 0.35s ease" }}>
        ...
        {/* ── Persistent top nav with theme toggle + manage booking ── */}
