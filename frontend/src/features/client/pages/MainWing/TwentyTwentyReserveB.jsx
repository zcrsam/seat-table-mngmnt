...
// FIX: ModalShell always uses a dark header gradient so text is always readable
...
// FIX: ModalHeader always uses a dark gradient so eyebrow/title text is always visible
...
// ─── MODAL 1: Guest Count ─────────────────────────────────────────────────────
// isStandalone: hides table row, shows just seat info, no guest count stepper
function ModalGuestCount({ seatData, tableData, mode, isStandalone, onContinue, onCancel, C, isDark }) {
  ...
  // Standalone: just show room + seat number + availability, no table row, no stepper
  if (isStandalone) {
    const infoRows = [
      ["Room",         ROOM,                                                                null],
  ...
  // Individual seat: show room + table + seat + availability
  const infoRows = [
    ["Room",         ROOM,                                                            null],
...
// Mobile Bottom Sheet — no "Type / Standalone" chip, just hide table chip for standalone
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 200, background: C.bottomSheet, borderTop: `1px solid ${C.borderAccent}`, borderRadius: "20px 20px 0 0", boxShadow: "0 -8px 32px rgba(0,0,0,0.28)", padding: "0 0 max(env(safe-area-inset-bottom), 12px) 0", animation: "slideUp 0.26s cubic-bezier(0.16,1,0.3,1)" }}>
      ...
      <div style={{ padding: "10px 16px 14px" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          ...
          {/* Table chip — hidden for standalone seats */}
          {!isStandalone && (
          ...
          <div style={{ flex: 1.4, padding: "8px 12px", borderRadius: 10, background: C.surfaceInput, border: `1px solid ${C.borderDefault}` }}>
            ...
            <div style={{ fontFamily: F.body, fontSize: 11, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>20/20 B</div>
          </div>
        </div>
...
export default function TwentyTwentyReserveB() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fallbackTimer = setTimeout(() => {
        if (!wsConnected) { console.log("[TwentyTwentyReserveB] WebSocket not active after 8s, starting polling"); startPolling(); }
      }, 8_000);
  ...
  // Display values — no "Standalone" label shown anywhere in the UI
  const displayTable = mode === "whole"
    ? (activeTable ? `Table ${activeTable.id}` : "—")
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", background: isDark ? "rgba(10,9,8,0.85)" : "rgba(247,244,238,0.90)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.borderAccent}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>20/20 Function Room B</div>
              </div>
            </div>
