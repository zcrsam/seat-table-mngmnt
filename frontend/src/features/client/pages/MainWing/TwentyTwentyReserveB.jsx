// src/features/client/pages/TwentyTwentyReserveB.jsx
...
// "rejected"              → "available" (reservation denied, seat freed)
...
// FIX: mergeApiStatusIntoLayout ported from AlabangReserve — handles both
// nested-seats format AND flat table/seat rows, and normalises statuses
function mergeApiStatusIntoLayout(localLayout, apiData) {
  ...
  apiTables.forEach(t => {
    // Nested seats format: { id, seats: [{id, status}, ...] }
    ...
    // Flat format: { table_number, seat_number, status }
  ...
  // Also merge standalone seats if present
...
// MODAL 1: Guest Count
// FIX: added isStandalone prop — when standalone, shows seat info without table row
function ModalGuestCount({ seatData, tableData, mode, isStandalone, onContinue, onCancel, C, isDark }) {
  ...
  // FIX: standalone seat — show only Room + Seat + Availability (no Table row)
  if (isStandalone) {
    return (
  ...
  // Normal individual seat info rows (with Table row)
  const infoRows = [
    ["Room",         ROOM,                                                            null],
...
// MODAL 2: Details
// FIX: added isStandalone prop — hides Table column from the summary bar when standalone
function ModalDetails({ tableData, seatData, mode, guests, isStandalone, onReview, onCancel, prefill, C, isDark, secondsLeft, onTimerExpired }) {
  ...
  // FIX: standalone hides Table column from the quick-summary bar
...
// MODAL 3: Review
// FIX: added isStandalone prop — hides Table row from the reservation details
function ModalReview({ form, guests, tableData, seatData, mode, isStandalone, onSubmit, onEdit, submitting, isRebook, rebookFrom, C }) {
  ...
  const reservationRows = [
    ...
    // FIX: only show Table row when not a standalone seat
    ...(!isStandalone && tableData ? [["Table", `Table ${tableData?.id ?? "T1"}`]] : []),
    ["Seat(s)", seatDisplay],
...
const buildQrValue = ({ refCode }) => {
  ...
  return `${url}/twenty-twenty-b/${String(refCode || "").trim()}`;
};
...
function ModalSuccess({ refCode, onBack, mode, guests, isRebook, bookingDetails, C, isDark }) {
  ...
  const handleSavePhoto = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement("a"); link.download = `bellevue-twenty20-b-${refCode || "ticket"}.png`; link.href = canvas.toDataURL("image/png"); link.click();
...
// Mobile Bottom Sheet
// FIX: added isStandaloneSeat prop — shows "Standalone" label instead of table
      ...
      <div style={{ padding: "10px 16px 14px" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1.4, padding: "8px 12px", borderRadius: 10, background: C.surfaceInput, border: `1px solid ${C.borderDefault}` }}>
            ...
            <div style={{ fontFamily: FONT, fontSize: 11, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>20/20 B</div>
          </div>
        </div>
...
export default function TwentyTwentyReserveB() {
  ...
  // FIX: centralised fetchAndMerge — used on mount and by polling fallback
  const fetchAndMerge = useCallback(async () => {
    ...
    } catch (err) { console.error("[TwentyTwentyReserveB] Failed to fetch seat status:", err); }
  }, []);
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      ...
      console.log("[TwentyTwentyReserveB] Starting polling fallback (10s interval)");
    ...
    try {
      ...
      console.warn("[TwentyTwentyReserveB] Echo init failed, falling back to polling:", err);
      startPolling();
      return () => stopPolling();
    }
    ...
    try {
      ...
      const fallbackTimer = setTimeout(() => {
        if (!wsConnected) {
          console.log("[TwentyTwentyReserveB] WebSocket not active after 8s, starting polling");
          startPolling();
        }
      }, 8_000);
      ...
      console.warn("[TwentyTwentyReserveB] Channel subscription failed, falling back to polling:", err);
      startPolling();
      return () => stopPolling();
    }
  }, [fetchAndMerge]);
  ...
  // FIX: detect if the currently-selected seat is a standalone seat
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Optimistic UI update — mark seat(s) as pending
      setTableData(prev => {
        if (!prev) return prev;
  ...
  const handleBack = () => {
    ...
    // Re-fetch after success to get authoritative server statuses
    fetchAndMerge();
  };
  ...
  // FIX: standalone shows "Standalone" instead of a table label
  ...
  // FIX: derive the tableData to pass to modals — null for standalone seats
          ...
          <div style={{ position: "relative", zIndex: 1, paddingTop: NAV_H }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", background: isDark ? "rgba(10,9,8,0.85)" : "rgba(247,244,238,0.90)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.borderAccent}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'Playfair Display','Times New Roman',serif", fontSize: 15, fontWeight: 600, color: C.textPrimary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>20/20 Function Room B</div>
              </div>
            </div>
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto", padding: isTablet ? "28px 24px" : "36px 48px" }}>
              ...
              <div style={{ marginBottom: 28, animation: "fadeUp 0.32s ease" }}>
                ...
                <p style={{ fontFamily: FONT, fontSize: 13.5, color: C.textSecondary, margin: 0, lineHeight: 1.70, maxWidth: 560 }}>
                  Book your preferred table in the 20/20 Function Room B. Select your reservation type and click on the map to get started.
                </p>
              </div>
                  ...
                  <div style={{ display: isTablet ? "grid" : "flex", gridTemplateColumns: isTablet ? "1fr 1fr" : undefined, flexDirection: isTablet ? undefined : "column", gap: 14 }}>
                    ...
                    <div style={{ background: C.surfaceBase, borderRadius: 12, border: `1px solid ${C.borderDefault}`, overflow: "hidden", boxShadow: isDark ? "0 4px 20px rgba(0,0,0,0.30)" : "0 2px 12px rgba(0,0,0,0.06)" }}>
                      ...
                      <div style={{ padding: "14px 16px" }}>
                        ...
                        {[
                          // FIX: standalone shows "Type / Standalone" instead of "Table / T1"
