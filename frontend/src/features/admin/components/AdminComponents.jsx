...
function DetailModal({ res, onClose, onApprove, onReject }) {
  ...
  // ── Compute accurate display values ──────────────────────────────────────
  const isWhole = res.type === "whole";

  // "Seat" row: for whole-table show "X / Y seats reserved", for individual show seat label
  const seatDisplay = isWhole
    ? `${res.guests}${res.tableCapacity ? ` / ${res.tableCapacity}` : ""} seats reserved`
    : (res.seat || "—");

  // "Type" row: show how many guests out of capacity, not just "Whole Table"
  ...
  if (confirm) {
    ...
    return (
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300 }}>
        <div style={{ background:"#FFFFFF", border:"1px solid rgba(201,168,76,0.3)", borderRadius:12, padding:"36px 40px", width:360, maxWidth:"90vw", textAlign:"center", boxShadow:"0 20px 60px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:12, color:"#666666", lineHeight:1.6, marginBottom:24 }}>{isApprove?`This will mark table as Reserved and notify ${res.name}.`:`This will release seat back to Available and notify ${res.name}.`}</div>
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.72)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
      <div style={{ background:"#FFFFFF", border:"1px solid rgba(201,168,76,0.3)", borderRadius:14, padding:"40px 44px", width:500, maxWidth:"90vw", boxShadow:"0 24px 80px rgba(0,0,0,0.6)", maxHeight:"90vh", overflowY:"auto", position:"relative" }}>
        ...
        <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:11, color:"#666666", marginBottom:16, letterSpacing:1 }}>Ref: {res.id} · Submitted {res.submittedAt}</div>
