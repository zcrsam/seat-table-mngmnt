...
function DetailModal({ res, onClose, onApprove, onReject }) {
  ...
  if (confirm) {
    ...
    return (
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300 }}>
        <div style={{ background:"#1B2A4A", border:"1px solid rgba(201,168,76,0.25)", borderRadius:12, padding:"36px 40px", width:360, maxWidth:"90vw", textAlign:"center", boxShadow:"0 20px 60px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:22, color:"#E8E2D4", fontWeight:"bold", marginBottom:8 }}>{isApprove?"Confirm Reservation?":"Reject Reservation?"}</div>
          <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:12, color:"#7A8A8A", lineHeight:1.6, marginBottom:24 }}>{isApprove?`This will mark table as Reserved and notify ${res.name}.`:`This will release seat back to Available and notify ${res.name}.`}</div>
          <div style={{ display:"flex", gap:10 }}>
            <button style={{ flex:1, padding:"12px 0", border:"1.5px solid #5A6A8A", background:"transparent", color:"#7A8A8A", borderRadius:6, fontFamily:"Montserrat, sans-serif", fontWeight:700, fontSize:11, letterSpacing:2, cursor:"pointer" }} onClick={()=>setConfirm(null)}>CANCEL</button>
            <button style={{ flex:1, padding:"12px 0", border:"none", background:isApprove?"#2E8B57":"#C0392B", color:"#fff", borderRadius:6, fontFamily:"Montserrat, sans-serif", fontWeight:700, fontSize:11, letterSpacing:2, cursor:"pointer" }} onClick={()=>{isApprove?onApprove(res.id):onReject(res.id);onClose();}}>{isApprove?"APPROVE":"REJECT"}</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.72)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
      <div style={{ background:"#141F30", border:"1px solid rgba(201,168,76,0.2)", borderRadius:14, padding:"40px 44px", width:500, maxWidth:"90vw", boxShadow:"0 24px 80px rgba(0,0,0,0.6)", maxHeight:"90vh", overflowY:"auto", position:"relative" }}>
        <button onClick={onClose} style={{ position:"absolute", top:16, right:16, background:"rgba(255,255,255,0.06)", border:"none", color:"#7A8A8A", width:32, height:32, borderRadius:"50%", cursor:"pointer", fontSize:15 }}>✕</button>
        ...
        <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:24, color:"#E8E2D4", fontWeight:"bold", marginBottom:4 }}>{res.name}</div>
        <div style={{ fontFamily:"Montserrat, sans-serif", fontSize:11, color:"#5A6A8A", marginBottom:16, letterSpacing:1 }}>Ref: {res.id} · Submitted {res.submittedAt}</div>
        ...
        {[["BOOKING INFORMATION",[["Room",res.room],["Table",res.table],["Seat",res.seat||"Whole Table"],["Guests",`${res.guests} guest${res.guests>1?"s":""}`],["Event Date",res.eventDate],["Event Time",res.eventTime||"Pending"],["Type",res.type==="whole"?"Whole Table":"Individual Seat"]]],["GUEST INFORMATION",[["Email",res.email],["Phone",res.phone],["Special Requests",res.specialRequests]]]].map(([section,rows])=>(
          <div key={section}>
            ...
            {rows.map(([k,v])=>(
              <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:"1px solid rgba(255,255,255,0.05)", fontFamily:"Montserrat, sans-serif", fontSize:12 }}>
                <span style={{ color:"#5A6A8A" }}>{k}</span>
                <span style={{ color:"#E8E2D4", fontWeight:600, textAlign:"right", maxWidth:"60%" }}>{v}</span>
              </div>
            ))}
          </div>
        ))}
