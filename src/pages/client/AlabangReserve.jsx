// src/pages/client/AlabangReserve.jsx
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import MainWingNavbar from "../../components/client/MainWingNavbar";

// ─── FIX: import SeatMap from the CORRECT shared path ─────────────────────────
// Was:  "../../components/client/SeatMap"  (didn't exist → white screen)
// Now:  "../../components/admin/SeatMap"   (the single shared SeatMap component)
import SeatMap, { STATUS_COLORS, STATUS_LABELS } from "../../components/admin/SeatMap";

// ─── FIX: import TABLE_T1 from the CORRECT data path ─────────────────────────
// Was:  "../../utils/seatMapData"   (wrong folder → module-not-found crash)
// Now:  "../../data/seatMapData"    (correct location)
...
const s = {
  ...
  legendDot: (color) => ({ width: 14, height: 14, borderRadius: 3, background: color }),
  legendText: { fontFamily: "Arial, sans-serif", fontSize: 10, color: "#333" },
  legendBadge: (color) => ({ fontFamily: "Arial, sans-serif", fontSize: 5, fontWeight: 700, letterSpacing: 1, color }),
  selCard: {
    background: "#fff",
    borderRadius: 8,
    padding: "6px 10px",
  ...
  selTitle: { fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 8, letterSpacing: 2, color: "#1B2A4A", marginBottom: 4 },
  selRow: { marginBottom: 2 },
  selLabel: { fontFamily: "Arial, sans-serif", fontSize: 8, letterSpacing: 1.5, color: "#666", fontWeight: 700 },
  selVal: { fontFamily: "Arial, sans-serif", fontSize: 10, color: "#1B2A4A", marginLeft: 6 },
  ctaBtn: (enabled) => ({
    marginTop: 6, width: "100%", padding: "6px 0",
    background: enabled ? "#C9A84C" : "#3A4A6A",
    color: enabled ? "#1B2A4A" : "#5A6A8A",
    border: "none", borderRadius: 4,
    fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 8, letterSpacing: 1.5,
    cursor: enabled ? "pointer" : "default", transition: "all 0.2s",
  }),
  policyCard: { background: "#fff", borderRadius: 8, padding: "6px 12px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" },
  policyTitle: { fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 9, letterSpacing: 2, color: "#1B2A4A", marginBottom: 4 },
  policyText: { fontFamily: "Arial, sans-serif", fontSize: 10, color: "#666", lineHeight: 1.4 },
  overlay: { position: "fixed", inset: 0, background: "rgba(27,42,74,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 },
  modalNavy: { background: "#1B2A4A", borderRadius: 12, padding: "36px 40px", width: 420, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.35)", color: "#fff", position: "relative" },
  modalTag: { fontFamily: "Arial, sans-serif", fontSize: 9, letterSpacing: 2, color: "#C9A84C", fontWeight: 700, marginBottom: 8 },
  modalTitle: { fontFamily: "Georgia, serif", fontSize: 26, fontWeight: "bold", color: "#fff", marginBottom: 4 },
  stepRow: { display: "flex", alignItems: "center", gap: 6, marginBottom: 24, marginTop: 6 },
  stepItem: (active, done) => ({ fontFamily: "Arial, sans-serif", fontSize: 9, letterSpacing: 1.5, fontWeight: 700, color: done ? "#C9A84C" : active ? "#fff" : "#5A6A8A", display: "flex", alignItems: "center", gap: 4 }),
  ...
  counterNum: { fontFamily: "Georgia, serif", fontSize: 52, fontWeight: "bold", color: "#fff", lineHeight: 1 },
  counterLabel: { fontFamily: "Arial, sans-serif", fontSize: 9, letterSpacing: 2, color: "#A0AABB", fontWeight: 700 },
  counterNote: { fontFamily: "Arial, sans-serif", fontSize: 11, color: "#A0AABB", marginBottom: 24, lineHeight: 1.5, textAlign: "center" },
  continueBtn: { width: "100%", padding: "14px 0", background: "#1B2A4A", color: "#fff", border: "2px solid #fff", borderRadius: 4, fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 2, cursor: "pointer", marginBottom: 10 },
  cancelBtn: { width: "100%", padding: "10px 0", background: "transparent", color: "#A0AABB", border: "none", fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: 2, cursor: "pointer" },
  formLabel: { fontFamily: "Arial, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: 1, color: "#A0AABB", marginBottom: 4, display: "block" },
  formInput: { width: "100%", padding: "9px 12px", borderRadius: 4, border: "1px solid #3A4A6A", background: "#0F1E38", color: "#fff", fontFamily: "Arial, sans-serif", fontSize: 12, marginBottom: 12, boxSizing: "border-box", outline: "none" },
  formRow: { display: "flex", gap: 10 },
  timerBar: { background: "#0F1E38", borderRadius: 6, padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, fontSize: 10, fontFamily: "Arial, sans-serif", color: "#A0AABB", letterSpacing: 1 },
  timerNum: { color: "#C9A84C", fontWeight: 700, fontSize: 14 },
  infoBadge: { background: "#0F1E38", borderRadius: 6, padding: "7px 12px", display: "flex", gap: 12, marginBottom: 16, fontSize: 11, fontFamily: "Arial, sans-serif", color: "#A0AABB" },
  ...
  reviewBtn: { width: "100%", padding: "13px 0", background: "#1B2A4A", color: "#fff", border: "2px solid #fff", borderRadius: 4, fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 2, cursor: "pointer", marginTop: 6 },
  reviewModal: { background: "#fff", borderRadius: 12, padding: "36px 40px", width: 440, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", position: "relative", maxHeight: "90vh", overflowY: "auto" },
  reviewTitle: { fontFamily: "Georgia, serif", fontSize: 26, fontWeight: "bold", color: "#1B2A4A", marginBottom: 4 },
  sectionLabel: { fontFamily: "Arial, sans-serif", fontSize: 9, letterSpacing: 2, fontWeight: 700, color: "#C9A84C", marginBottom: 10, marginTop: 16 },
  reviewRow: { display: "flex", justifyContent: "space-between", borderBottom: "1px solid #F0EDE4", padding: "8px 0", fontFamily: "Arial, sans-serif", fontSize: 12 },
  reviewKey: { color: "#888" },
  reviewVal: { color: "#1B2A4A", fontWeight: 600, textAlign: "right" },
  pendingBadge: { background: "#FFF3E0", color: "#E8A838", borderRadius: 3, padding: "2px 8px", fontFamily: "Arial, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: 1 },
  editBtn: { flex: 1, padding: "12px 0", border: "2px solid #1B2A4A", background: "transparent", color: "#1B2A4A", borderRadius: 4, fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: 2, cursor: "pointer" },
  submitBtn: { flex: 1, padding: "12px 0", border: "none", background: "#1B2A4A", color: "#fff", borderRadius: 4, fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: 2, cursor: "pointer" },
  successModal: { background: "#fff", borderRadius: 12, padding: "48px 40px", width: 380, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", textAlign: "center" },
  checkCircle: { width: 56, height: 56, borderRadius: "50%", background: "#E8F5EE", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28, color: "#4CAF79" },
  successTitle: { fontFamily: "Georgia, serif", fontSize: 26, fontWeight: "bold", color: "#1B2A4A", marginBottom: 10 },
  successNote: { fontFamily: "Arial, sans-serif", fontSize: 12, color: "#777", lineHeight: 1.6, marginBottom: 20 },
  refCode: { background: "#1B2A4A", color: "#fff", borderRadius: 4, padding: "13px 20px", fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 15, letterSpacing: 3, marginBottom: 8, display: "block", textAlign: "center" },
  refNote: { fontFamily: "Arial, sans-serif", fontSize: 10, color: "#AAA", marginBottom: 20, letterSpacing: 1 },
  backMapBtn: { width: "100%", padding: "11px 0", border: "2px solid #1B2A4A", background: "transparent", color: "#1B2A4A", borderRadius: 4, fontFamily: "Arial, sans-serif", fontWeight: 700, fontSize: 11, cursor: "pointer" },
};
  ...
  return (
    <div style={s.overlay}>
      <div style={s.reviewModal}>
        <div style={{ fontFamily: "Arial, sans-serif", fontSize: 9, letterSpacing: 2, color: "#C9A84C", fontWeight: 700, marginBottom: 6 }}>STEP 3 OF 3 · TABLE RESERVATION</div>
...
export default function AlabangReserve() {
  ...
  // Load persistent seat map (falls back to default TABLE_T1)
  const [tableData, setTableData] = useState(() => getRoomData(WING, ROOM, TABLE_T1));

  // Subscribe to live updates pushed by the admin SeatMap in editMode
  useEffect(() => {
  ...
  const getResponsiveStyles = () => {
    ...
    if (width < 480)  return { page: { padding: "16px 12px", maxWidth: "100%" }, layout: { flexDirection: "row", gap: 12 }, mapCard: { padding: 16, minWidth: "auto", flex: 1 }, rightPanel: { width: "200px", gap: 12, flexShrink: 0 }, pageTitle: { fontSize: 22 } };
    if (width < 768)  return { page: { padding: "24px 20px", maxWidth: "100%" }, layout: { flexDirection: "row", gap: 16 }, mapCard: { padding: 20, minWidth: "auto", flex: 1 }, rightPanel: { width: "240px", gap: 14, flexShrink: 0 }, pageTitle: { fontSize: 26 } };
    return { page: { padding: "32px 40px", maxWidth: 1100 }, layout: { flexDirection: "row", gap: 28 }, mapCard: { padding: 24, minWidth: 320, flex: 1 }, rightPanel: { width: "280px", gap: 14, flexShrink: 0 }, pageTitle: { fontSize: 30 } };
  };
    ...
    <div style={s.root}>
      ...
      <div style={{ ...s.page, ...responsive.page }}>
        ...
        <div style={{ ...s.toggleBar, flexDirection: "row", alignItems: "center", marginBottom: 0, flexWrap: isMobile ? "wrap" : "nowrap" }}>
          <span style={{ ...s.toggleLabel, fontSize: isMobile ? 8 : 10, whiteSpace: "nowrap" }}>I WANT TO RESERVE A:</span>
          <div style={s.togglePillGroup}>
            <button style={{ ...s.togglePillBtn(mode === "whole"), padding: isMobile ? "6px 14px" : "10px 28px", fontSize: isMobile ? 8 : 10, marginLeft: isMobile ? 8 : 180 }}
              onClick={() => { setMode("whole"); setSelectedSeat(null); }}>WHOLE TABLE</button>
            <button style={{ ...s.togglePillBtn(mode === "individual"), padding: isMobile ? "6px 14px" : "10px 28px", fontSize: isMobile ? 8 : 10 }}
        ...
        <div style={{ ...s.layout, ...responsive.layout, overflowX: "auto" }}>
          ...
          <div style={{ ...s.mapCard, ...responsive.mapCard, minWidth: "300px" }}>
            ...
            {/* Uses the shared SeatMap component (view mode only — no editMode prop) */}
            <SeatMap
              tableData={tableData}
              mode={mode}
          ...
          <div style={{ ...s.rightPanel, ...responsive.rightPanel, minWidth: "200px" }}>
            ...
            <div style={{ ...s.selCard, padding: isMobile ? "12px 16px" : "16px 20px" }}>
              ...
              <div style={s.selRow}><span style={{ ...s.selLabel, fontSize: isMobile ? 8 : 9 }}>SEAT:</span><span style={{ ...s.selVal, fontSize: isMobile ? 11 : 12 }}>{selSeat}</span></div>
              <div style={s.selRow}><span style={{ ...s.selLabel, fontSize: isMobile ? 8 : 9 }}>TABLE:</span><span style={{ ...s.selVal, fontSize: isMobile ? 11 : 12 }}>{selTable}</span></div>
              {mode === "whole" && (
                <div style={{ marginTop: 6 }}>
                  <button style={{ ...s.ctaBtn(true), fontSize: isMobile ? 7 : 8 }} onClick={() => setModal("guestCount")}
                    onMouseEnter={e => { e.target.style.background = "#B89635"; }}
                    onMouseLeave={e => { e.target.style.background = "#C9A84C"; }}>RESERVE THIS TABLE</button>
                </div>
              )}
              {mode === "individual" && (
                <button style={{ ...s.ctaBtn(canProceed), fontSize: isMobile ? 7 : 8 }} onClick={canProceed ? () => setModal("guestCount") : undefined}>
                  RESERVE THIS SEAT
                </button>
              )}
            </div>
      ...
      {/* MODALS */}
      {modal === "guestCount" && <ModalGuestCount tableData={tableData} mode={mode} onContinue={handleGuestContinue} onCancel={() => setModal(null)} />}
      {modal === "details"    && <ModalDetails tableData={tableData} mode={mode} guests={guests} onReview={handleReview} onCancel={() => setModal(null)} />}
      {modal === "review"     && formData && <ModalReview form={formData} guests={guests} tableData={tableData} onSubmit={handleSubmit} onEdit={() => setModal("details")} />}
